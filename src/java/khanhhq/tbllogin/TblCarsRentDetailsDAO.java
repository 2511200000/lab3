...
public class TblCarsRentDetailsDAO implements Serializable {
    ...
    public boolean createOrderDetails(int billID, String carID, int quantity, float price, float total,String rentalDate,String returnDate) throws NamingException, SQLException {
        Connection con = null;
        PreparedStatement stm = null;
         try {
            con = DbHelp.makeConnection();
            if (con != null) {
                String sql = "Insert into tblCarsRentDetails(billID,carID,quantity,price,total,rentalDate,returnDate)"
                        + "Values(?, ?, ?, ?, ?, ?, ?)";
